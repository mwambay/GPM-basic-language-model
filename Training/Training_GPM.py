...
file = open(rf"{chemin_parent}\training data\texte_representatif.txt", encoding='utf-8')
...
for token in tableau_partiel:
    if token == "<text_stop>":
        ...
        vectorisation.append(token)
file = open(rf"{chemin_parent}\training data\training_data.json", 'w', encoding='utf-8')
...
def TeDS_Training():
    with open(rf'{chemin_parent}\training data\training_data.json', 'r', encoding="utf-8") as file:
    ...
    with open(rf'{chemin_parent}\model\vectors\GP-Model.json', 'w',encoding= 'utf-8') as file:
    ...
    with open(rf'{chemin_parent}\model\vectors\vecteur_multi_dimentionnel_back.json', 'w') as file:
        json.dump(relation_sequence_back_propagation, file)
        file.close()
    with open(rf'{chemin_parent}\model\vectors\vecteur_multi_dimentionnel.json', 'w') as file:
    ...
    def attention():
        ...
        with open(rf"{chemin_parent}\model\vectors\element_synonyme.json", 'w') as file:
